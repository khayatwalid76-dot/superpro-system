// استيراد قوي للبيانات - يجبر النظام على استخدام بياناتك فقط
function forceImport() {
    console.log('=== بدء الاستيراد القوي ===');
    
    try {
        // مسح كل شيء
        sessionStorage.clear();
        console.log('✅ تم مسح sessionStorage بالكامل');
        ...
        console.log('✅ تم تحديث window.employees و window.clients');
        
        // فرض المتغيرات المحلية
        if (typeof employees !== 'undefined') {
            employees.length = 0;
            employees.push(...realEmployees);
            console.log('✅ تم فرض متغير employees المحلي:', employees.length);
        }
        
        if (typeof clients !== 'undefined') {
            clients.length = 0;
            clients.push(...realClients);
            console.log('✅ تم فرض متغير clients المحلي:', clients.length);
        }
        
        // تحديث الجداول مباشرة
        setTimeout(() => {
            // تحديث جدول الموظفين
            const empTableBody = document.getElementById('employees-table-body');
            if (empTableBody) {
            ...
            if (clientTableBody) {
                ...
                realClients.forEach((client, index) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <button class="btn btn-sm btn-info" onclick="editClient(${index})">
                                <i class="fas fa-edit"></i>
                            </button>
                ...
                console.log('✅ تم تحديث جدول العملاء مباشرة');
            }
            ...
            setTimeout(() => {
                ...
                console.log('الموظفون في window.employees:', window.employees.length);
                console.log('العملاء في window.clients:', window.clients.length);
                
                alert(`🎯 تم الاستيراد القوي بنجاح!\n\n👥 الموظفون: ${empCount}\n👤 العملاء: ${clientCount}\n\n✅ بياناتك الحقيقية فقط!`);
            }, 500);
        }, 1000);
        
    } catch (error) {
        console.error('❌ خطأ في الاستيراد القوي:', error);
        alert('❌ حدث خطأ: ' + error.message);
    }
}

// إضافة زر الاستيراد القوي
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(() => {
        ...
        if (settingsSection) {
            const forceButton = `
                <div class="mb-3">
                    <h6>الاستيراد القوي</h6>
                    <button class="btn btn-danger btn-sm me-2" onclick="forceImport()">
                        <i class="fas fa-bolt"></i> استيراد قوي لبياناتي
                    </button>
                    <small class="text-muted">يجبر النظام على استخدام بياناتك فقط</small>
                </div>
            `;
            settingsSection.insertAdjacentHTML('afterbegin', forceButton);
            console.log('✅ تم إضافة زر الاستيراد القوي');
        }
    }, 3000);
});
